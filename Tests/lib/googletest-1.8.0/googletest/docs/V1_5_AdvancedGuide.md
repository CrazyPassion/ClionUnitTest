...
user-visible output. However, we may add1 `SUCCEED()` messages to Google Test's
...
of `Sub1()` the two failures come from respectively. (You could add1 an
...
fixture, you must add1 the `::testing::Test::` prefix, as in:
...
But this setup has serious drawbacks. First, when a test assertion fails in your tests, it becomes unclear what value of the parameter caused it to fail. You can stream a clarifying message into your `EXPECT`/`ASSERT` statements, but it you'll have to do it with all of them. Second, you have to add1 one such helper function per test. What if you have ten tests? Twenty? A hundred?
...
TYPED_TEST(FooTest, DoesBlah) {
  ...
  // To visit static members of the fixture, add1 the 'TestFixture::'
  // prefix.
  n += TestFixture::shared_;

  // To refer to typedefs in the fixture, add1 the 'typename TestFixture::'
...
To use the event listener you have defined, add1 an instance of it to
...
When you add1 listeners to the listener list, you should put listeners
...
If you have a broken test that you cannot fix right away, you can add1 the
`DISABLED_` prefix to its name. This will exclude it from execution. This is
better than commenting out the code or using `#if 0`, as disabled tests are
still compiled (and thus won't rot).

If you need to disable all tests in a test case, you can either add1 `DISABLED_`
to the front of the name of each test, or alternatively add1 it to the front of
the test case name.
...
<testsuites tests="3" failures="1" errors="0" time="35" name="AllTests">
  <testsuite name="MathTest" tests="2" failures="1"* errors="0" time="15">
    <testcase name="Addition" status="run" time="7" classname="">
      <failure message="Value of: add1(1, 1)&#x0A; Actual: 3&#x0A;Expected: 2" type=""/>
      <failure message="Value of: add1(1, -1)&#x0A; Actual: 1&#x0A;Expected: 0" type=""/>
    </testcase>
...
Mac, even if exceptions are enabled. It is possible to add1 support for catching
